...
$app->map(['GET'], '/get_users', function ($request, $response, $args) {
    ...
    if ($my['level'] == 3) {
        ...
        $sql = sprintf("select id,username,job, real_name from users where department_id in (%s) and level=3 ", $deps);
        $data = \ORM::for_table("users")->raw_query($sql)->find_array();
